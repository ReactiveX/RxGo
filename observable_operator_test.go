...
func Test_Observable_BufferWithTime(t *testing.T) {
	...
	observe := Just(1, 2, 3, 4, 5, 6, 7, 8, 9)(WithContext(ctx)).Scan(
		func(ctx context.Context, i3 interface{}, v interface{}) (interface{}, error) {
			if v.(int)%3 == 0 {
...
func Test_Observable_BufferWithTimeOrCount(t *testing.T) {
	...
	Assert(ctx, t, JustItem((<-observe).V), HasItems(0, 1, 2)) // triggered by timeout
	Assert(ctx, t, JustItem((<-observe).V), HasItems(3, 4, 5)) // triggered by count
	Assert(ctx, t, JustItem((<-observe).V), HasItems(6))       // triggered by timeout
...
func Test_Observable_WindowWithTime_Error(t *testing.T) {
	...
	observe := Just(
		0, 1, 2, errFoo, 4,
	)(WithContext(ctx)).WindowWithTime(
		WithDuration(3*time.Second),
		WithContext(ctx),
	).Observe()
	Assert(ctx, t, (<-observe).V.(Observable), HasItems(0, 1, 2), HasError(errFoo))
	cancel()
}
...
func Test_Observable_WindowWithTime_ContinueOnError(t *testing.T) {
	...
	Assert(ctx, t, (<-observe).V.(Observable), HasItems(0, 1, 2, 4), HasError(errFoo))
	cancel()
}
...
func Test_Observable_WindowWithTimeOrCount(t *testing.T) {
	...
	Assert(ctx, t, (<-observe).V.(Observable), HasItems(0, 1))
	Assert(ctx, t, (<-observe).V.(Observable), HasItems(2, 3))
	Assert(ctx, t, (<-observe).V.(Observable), HasItems(4))
	Assert(ctx, t, (<-observe).V.(Observable), HasItems(5, 6))
	Assert(ctx, t, (<-observe).V.(Observable), HasItems(7))
...
func Test_Observable_WindowWithTimeOrCount_ContinueOnError(t *testing.T) {
	...
	observe := Just(1, errFoo, 2, 3)(
		WithContext(ctx), WithErrorStrategy(ContinueOnError),
	).WindowWithTimeOrCount(
